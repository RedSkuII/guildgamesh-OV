...
// Standard 98 Dune Awakening Resources
// These will be the default for all guilds
...
async function resetGuildData() {
  try {
    ...
    // Step 1: Get existing guilds from database
    console.log('📋 Checking for existing guilds...')
    const existingGuilds = await client.execute('SELECT id, discord_guild_id, title FROM guilds')
    
    if (existingGuilds.rows.length === 0) {
      console.log('⚠️ No guilds found in database!')
      console.log('Please ensure guilds are created via the bot first.\n')
      return
    }

    console.log(`Found ${existingGuilds.rows.length} guild(s):`)
    existingGuilds.rows.forEach(g => {
      console.log(`  - ${g.title} (ID: ${g.id}, Discord: ${g.discord_guild_id})`)
    })
    console.log()
    ...
    console.log('✅ Cleared resource_history, leaderboard, and resources tables\n')

    // Step 3: Create standard resources for each guild
    console.log(`📦 Creating ${STANDARD_RESOURCES.length} standard resources for each guild...\n`)

    for (const guild of existingGuilds.rows) {
      console.log(`Processing guild: ${guild.title}...`)
      ...
      for (const resource of STANDARD_RESOURCES) {
        ...
        await client.execute({
          ...
          args: [
            resourceId,
            guild.id, // in-game guild ID (e.g., 'house-melange')
            resource.name,
            0, // Start at 0
            resource.description,
            resource.category,
            resource.icon,
            'critical', // 0/1000 = critical status
            1000, // Target quantity
            resource.multiplier,
            'System',
            now,
            now
      ...
      console.log(`✅ Created ${STANDARD_RESOURCES.length} resources for ${guild.title}`)
    }
    ...
    console.log('\n✅ Guild data reset complete!')
    console.log(`\nAll guilds now have ${STANDARD_RESOURCES.length} standard resources:`)
...
// Export function for use in bot initialization
async function createStandardResourcesForGuild(guildId, guildTitle) {
  try {
    const client = createClient({
      url: process.env.TURSO_DATABASE_URL,
      authToken: process.env.TURSO_AUTH_TOKEN || undefined,
    ...
    console.log(`\n📦 Creating standard resources for new guild: ${guildTitle}`)
    ...
    console.log(`✅ Created ${STANDARD_RESOURCES.length} resources for ${guildTitle}`)
    return { success: true, count: STANDARD_RESOURCES.length }

  } catch (error) {
    console.error('❌ Error creating resources for guild:', error.message)
...
module.exports = { createStandardResourcesForGuild, STANDARD_RESOURCES }
